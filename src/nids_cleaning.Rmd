...
w1_bh <-  wave_1_adult %>%
  ...
  rename(child_living_num = w1_a_bhlive_n,
         child_alive_num = w1_a_bhali_n,
         child_dead_num = w1_a_bhdth_n,
         child_1_birth_year = w1_a_bhdob_y1,
...
w1_bh <-  w1_bh %>%
  mutate(num_births = rowSums((w1_bh[,2:4]), na.rm = TRUE))

# Number of births for which birth timing data is available
w1_birthyears <-  w1_bh %>%
  select(ends_with("birth_year")) %>%
  sjmisc::row_count(count = NA, var = "num_na", append = TRUE)

w1_bh <-  w1_bh %>%
  mutate(num_bhdata = 17 - w1_birthyears$num_na)
